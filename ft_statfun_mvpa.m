function [stat, cfg] = ft_statfun_mvpa(cfg, dat, design)
...
if isempty(cfg.mvpa.model)
  ...
  elseif any(unique(design(cfg.uvar,:)) > 1000) % Hack to be able to test on separate data
      subs = unique(design(cfg.uvar,:));
      subs = subs(subs < 1000);
      perf = zeros(length(subs), size(dat,3));
      perf_std = perf;
      result = cell(length(subs),1);
      for sub = subs
          current_design_train = design(cfg.ivar,design(cfg.uvar,:) == sub);
          current_dat_train = dat(design(cfg.uvar,:) == sub,:,:);
          current_design_test = design(cfg.ivar,design(cfg.uvar,:) == (sub+1000)) - 2;
          current_dat_test = dat(design(cfg.uvar,:) == (sub+1000),:,:);
          [perf(sub,:), result{sub}] = mv_classify(cfg.mvpa, current_dat_train, current_design_train, current_dat_test, current_design_test);
          if iscell(result{sub}.perf_std)
              perf_std(sub,:) = result{sub}.perf_std{sub};
          else
              perf_std(sub,:) = result{sub}.perf_std;
          end
      end
  else
      perf = zeros(length(unique(design(cfg.uvar,:))), size(dat,3));
      perf_std = perf;
      result = cell(length(unique(design(cfg.uvar,:))),1);
      for sub = unique(design(cfg.uvar,:))
          current_design = design(cfg.ivar,design(cfg.uvar,:) == sub);
          current_dat = dat(design(cfg.uvar,:) == sub,:,:);
          [perf(sub,:), result{sub}] = mv_classify(cfg.mvpa, current_dat, current_design);
          if iscell(result{sub}.perf_std)
              perf_std(sub,:) = result{sub}.perf_std{sub};
          else
              perf_std(sub,:) = result{sub}.perf_std;
...
if has_dimord
  dimord = strrep(result{1}.perf_dimension_names, ' ', '');
...
for mm=1:numel(perf)
  ...
  % Performance metric
  stat.(cfg.mvpa.metric{mm}) = mean(perf{mm},1);
  stat.([cfg.mvpa.metric{mm} '_all_subs']) = perf{mm};
  stat.([cfg.mvpa.metric{mm} '_std']) = std(perf{mm},1);
  stat.([cfg.mvpa.metric{mm} '_std_all_subs']) = perf_std{mm};
...
stat.stat = stat.(cfg.mvpa.metric{1})'; % rename the auc field, added by A.S
% cfg.avgoverchan = 'yes';
end
