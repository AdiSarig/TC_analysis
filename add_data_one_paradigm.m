...
if strcmp(paradigm, 'IB')
    ...
    for ind = 1:length(noise_cond)
        ...
        for sub = sub_inds
            ...
            voltage_van = zeros(num_trials,1);
            for trial = 1:num_trials
                current_trial = data.(noise_cond{ind})(sub).EEG.trial{trial};
                voltage_p300(trial) = mean(current_trial(p3_channel_vec,p3_t_min:p3_t_max), 'all');
                voltage_van(trial) = mean(current_trial(van_channel_vec,van_t_min:van_t_max), 'all');
            end
            noise_p300_for_lmm = [noise_p300_for_lmm; struct('phase', ['phase_', noise_cond{ind}(2)],...
                'voltage', num2cell(voltage_p300), 'sub_num', data_avg.(noise_cond{ind})(sub).sub_num)];
            noise_van_for_lmm = [noise_van_for_lmm; struct('phase', ['phase_', noise_cond{ind}(2)],...
                'voltage', num2cell(voltage_van), 'sub_num', data_avg.(noise_cond{ind})(sub).sub_num)];
        end
    end
end
